package org.fossify.messages.arj.extensions
...
import org.fossify.messages.arj.databases.MessagesDatabase
import org.fossify.messages.arj.helpers.AttachmentUtils.parseAttachmentNames
import org.fossify.messages.arj.helpers.Config
import org.fossify.messages.arj.helpers.FILE_SIZE_NONE
import org.fossify.messages.arj.helpers.MAX_MESSAGE_LENGTH
import org.fossify.messages.arj.helpers.MESSAGES_LIMIT
import org.fossify.messages.arj.helpers.MessagingCache
import org.fossify.messages.arj.helpers.NotificationHelper
import org.fossify.messages.arj.helpers.ShortcutHelper
import org.fossify.messages.arj.helpers.generateRandomId
import org.fossify.messages.arj.helpers.refreshConversations
import org.fossify.messages.arj.interfaces.AttachmentsDao
import org.fossify.messages.arj.interfaces.ConversationsDao
import org.fossify.messages.arj.interfaces.DraftsDao
import org.fossify.messages.arj.interfaces.MessageAttachmentsDao
import org.fossify.messages.arj.interfaces.MessagesDao
import org.fossify.messages.arj.messaging.MessagingUtils
import org.fossify.messages.arj.messaging.MessagingUtils.Companion.ADDRESS_SEPARATOR
import org.fossify.messages.arj.messaging.SmsSender
import org.fossify.messages.arj.models.Attachment
import org.fossify.messages.arj.models.Conversation
import org.fossify.messages.arj.models.Draft
...
import org.fossify.messages.arj.models.RecycleBinMessage
...
fun Context.removeAllSpamConversations(callback: (() -> Unit)? = null) {
    ensureBackgroundThread {
        try {
            for (conversation in conversationsDB.getAllSpam()) {
                deleteConversation(conversation.threadId)
            }
           toast(R.string.spam_emptied_successfully)
            callback?.invoke()
        } catch (_: Exception) {
            toast(org.fossify.commons.R.string.unknown_error_occurred)
        }
...
fun Context.cleanupOldSpamMessages() {
    ensureBackgroundThread {
        try {
            val retentionDays = config.spamRetentionDays
            val messageLimit = config.spamMessageLimit
            val threadsToDelete = mutableSetOf<Long>()

            // Time-based cleanup: Delete spam older than X days (if not 0 = never)
            if (retentionDays > 0) {
                val cutoffDate = System.currentTimeMillis() - (retentionDays * 24 * 60 * 60 * 1000L)
                val oldSpamThreads = conversationsDB.getOldSpamThreadIds(cutoffDate)
                threadsToDelete.addAll(oldSpamThreads)
            }

            // Count-based cleanup: Keep only last N messages (if not 0 = unlimited)
            if (messageLimit > 0) {
                val excessSpamThreads = conversationsDB.getExcessSpamThreadIds(messageLimit)
                threadsToDelete.addAll(excessSpamThreads)
            }
            ...
            // Update last cleanup check time
            config.lastSpamCleanupCheck = System.currentTimeMillis()
        } catch (_: Exception) {
...
fun Context.deleteConversation(threadId: Long) {
    if (config.useRecycleBin) {
        val messages = messagesDB.getThreadMessages(threadId)
        messages.forEach { msg ->
            moveMessageToRecycleBin(msg.id)
        }
        refreshConversations()
        return
    }
...
fun Context.insertOrUpdateConversation(
    ...
    if (cachedConv != null) {
        // Preserve user-set flags that shouldn't be overwritten by system sync
        updatedConv = updatedConv.copy(
            title = if (cachedConv.usesCustomTitle) cachedConv.title else conversation.title,
            usesCustomTitle = cachedConv.usesCustomTitle,
            isArchived = cachedConv.isArchived,
            isSpam = cachedConv.isSpam
        )
    }
