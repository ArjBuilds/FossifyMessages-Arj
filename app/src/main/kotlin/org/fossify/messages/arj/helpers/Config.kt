...
class Config(context: Context) : BaseConfig(context) {
    ...
    var spamRetentionDays: Int
        get() = prefs.getInt(SPAM_RETENTION_DAYS, 30)
        set(spamRetentionDays) = prefs.edit().putInt(SPAM_RETENTION_DAYS, spamRetentionDays).apply()

    var spamMessageLimit: Int
        get() = prefs.getInt(SPAM_MESSAGE_LIMIT, 100)
        set(spamMessageLimit) = prefs.edit().putInt(SPAM_MESSAGE_LIMIT, spamMessageLimit).apply()

    var lastSpamCleanupCheck: Long
        get() = prefs.getLong(LAST_SPAM_CLEANUP_CHECK, 0L)
        set(lastSpamCleanupCheck) = prefs.edit().putLong(LAST_SPAM_CLEANUP_CHECK, lastSpamCleanupCheck).apply()
}
